...
use syn::{parse_macro_input, ImplItem, ImplItemMethod, ItemImpl};
...
fn lua_field_reg(item: &ImplItemMethod, attr: FieldAttr) -> TokenStream {
    ...
    let lua_name = attr
        .lua_name
        .map(|v| v.to_string())
        .unwrap_or_else(|| lua_name.to_string());
...
pub fn lua_impl(
    ...
    quote!(
        #item
        impl #generics mlua::UserData for #ty {
            fn add_methods<M: mlua::UserDataMethods<Self>>(methods: &mut M) {
                #methods
            }
            fn add_fields<M: mlua::UserDataFields<Self>>(fields: &mut M) {
                #fields
            }
        }
    )
