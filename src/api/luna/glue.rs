...
use crate::api::luna::glue::methods::GlueUserDataMethods;
...
impl<V: UserData> ToGlue for V {
	fn glue(&mut self) -> Glue<Self> { Glue::new(self) }
}
...
impl<'a, V: UserData> Glue<'a, V> {
	...
	pub fn lua(self) -> LuaGlue<V> { self.raw }
}
...
impl<V: UserData> LuaGlue<V> {
	pub fn new_raw(value: *mut V) -> LuaGlue<V> { LuaGlue { value } }
}

impl<V: UserData> Clone for LuaGlue<V> {
	fn clone(&self) -> Self { LuaGlue { value: self.value } }
}
